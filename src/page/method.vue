    ...
    <div class="custom-tree-container bdd_main">
        ...
        <el-tree :data="data5" node-key="id" ref="tree" :load="loadNode" lazy :expand-on-click-node="false" :filter-node-method="filterNode">
            <span class="custom-tree-node" slot-scope="{ node, data }">
                ...
                <span class='bdd_aids'>
                    <el-button type="text" size="mini" v-if="node.level<3" @click="() => append(node,data)">
                        增加
                    </el-button>
                    <el-button type="text" size="mini" v-if="node.level>1" @click="() => remove(node, data)">
                        刪除
                    </el-button>
                    <el-button type="text" size="mini" v-if="node.level>1" @click="() =>updata(node,data)">
                        修改
                    </el-button>
                </span>
            </span>
...
.custom-tree-node[data-v-e8a19380] {
    font-size: 14px;
}
</style>
